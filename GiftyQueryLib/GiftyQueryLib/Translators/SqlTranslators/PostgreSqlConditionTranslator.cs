...
namespace GiftyQueryLib.Translators.SqlTranslators
{
    ...
    public class PostgreSqlConditionTranslator : BaseConditionTranslator
    {
        ...
        private readonly Stack<KeyValuePair<string, string>> autogeneratedAliases;

        public PostgreSqlConditionTranslator(PostgreSqlConfig config, PostgreSqlFunctions func) : base()
        {
            this.aliases = new();
            this.autogeneratedAliases = new();
            this.config = config;
        ...
        public void AddAutoGeneratedAlias(string key, string value)
        {
            autogeneratedAliases.Push(new(key, value));
        }
            ...
            Type? extraType = null, SelectorConfig? config = null) where TItem : class
        {
            ...
            if (body is NewExpression newExpression && newExpression.Type.Name.Contains("Anonymous"))
            {
                ...
                {
                    string? paramName = config.UseAliases ? newExpression?.Members?[i]?.Name?.ToString()?.ToCaseFormat(caseConfig) : null;
        ...
        protected virtual string ParseMethodCallExpression(MethodCallExpression callExp, string? paramName = null, bool isSelectorParsing = false)
        {
            ...
            {
                return autogeneratedAliases.Pop().Value;
            }
